...
def detect_jul_2025(text):
    """Detecta datas (ddmmaaaa ou aaaammdd) e confere Jul/2025."""
...
def ia_priorizar_e_sugerir(findings_df_list, meta):
    ...
    # tenta usar chave (se houver); senão, cai no fallback determinístico
        ...
        with pd.ExcelWriter(out, engine="openpyxl") as w:
            ...
            if df_agg.empty:
                df_base = pd.DataFrame(columns=[
                    "codigo","desc_sigtap","vl_sh","vl_sa","vl_opm","vl_unit_total","qtd_total","qtd_julho","valor_total_estimado"
                ])
            else:
                df_base = (
